...
# 差分検出ではユーザーが選択したローカルTIFFを扱うため、Pillowの巨大画像保護で止めない
...
WINDOW_TITLE = "Fast TIFF Viewer v1.8.0"
...
DIFF_TIFF_BIGTIFF = True                # 4GB超の差分TIFFも保存できるBigTIFF形式を使う
DIFF_NO_CHANGE_TEXT = "差分検出なし"     # 完全一致ページへ表示する文字
DIFF_NO_CHANGE_FONT_FAMILY = "Meiryo"   # 完全一致表示に使用する日本語フォント
DIFF_NO_CHANGE_FONT_SIZE_RATIO = 1.0 / 15.0  # 画像高さに対するフォントサイズ比
DIFF_NO_CHANGE_TEXT_COLOR = (255, 0, 0)  # 完全一致表示の文字色(RGB)
...
def _load_diff_vips_page(file_path: str, page_index: int) -> pyvips.Image:
    """差分処理用の1ページを順次読み込みで開く。"""
    return _normalize_diff_vips_image(pyvips.Image.new_from_file(
        file_path,
        access="sequential",
        sequential=True,
        autorotate=True,
...
def _diff_no_change_font_size(image_height: int) -> int:
    """画像高さから完全一致表示のフォントサイズ(px)を求める。"""
    return max(1, int(round(image_height * DIFF_NO_CHANGE_FONT_SIZE_RATIO)))


def _add_diff_no_change_text(image: pyvips.Image) -> pyvips.Image:
    """完全一致ページの中央へ赤字メッセージを合成する。"""
    font_size = _diff_no_change_font_size(image.height)
    text_mask = pyvips.Image.text(
        DIFF_NO_CHANGE_TEXT,
        font=f"{DIFF_NO_CHANGE_FONT_FAMILY} {font_size}",
        dpi=72,
        width=image.width,
        align="centre",
    )
    # 72dpiを指定し、Pangoのポイント数と画像上のピクセル数を一致させる
    text_mask = text_mask.gravity(
        "centre",
        image.width,
        image.height,
        extend="black",
    )
    result = text_mask.ifthenelse(
        list(DIFF_NO_CHANGE_TEXT_COLOR),
        image,
        blend=True,
    )
...
def _diff_vips_pages_are_identical(old_image: pyvips.Image, new_image: pyvips.Image) -> bool:
    """同一寸法の2ページが画素単位で完全一致するか判定する。"""
    # RGBのいずれかが異なる画素だけを差分対象にする
    changed = (old_image != new_image).bandbool("or")
    # 1バンドの比較マスクをlibvips側で走査し、Pythonへの画素転送を避ける
    return changed.max() == 0
...
def _diff_vips_tiff_save_options() -> dict:
    """差分TIFFをlibvipsで保存するときの共通オプションを返す。"""
    options = {"compression": DIFF_TIFF_COMPRESSION}
    if DIFF_TIFF_BIGTIFF:
        options["bigtiff"] = True
    return options


def _open_diff_pil_page(page_path: Path):
    """Pillowの画素数上限を無効化して差分TIFFの一時ページを開く。"""
    # テストや外部コードが上限値を戻していても、差分結合時はサイズ制限を掛けない
    PILImage.MAX_IMAGE_PIXELS = None
    return PILImage.open(page_path)
...
def _write_diff_pages_tiff(diff_pages, temp_path: Path, progress_callback=None):
    ...
    if not has_variable_sizes:
        ...
        joined.write_to_file(str(temp_path), **_diff_vips_tiff_save_options())
    ...
    try:
        ...
        for page_index, page in enumerate(diff_pages):
            ...
            page.write_to_file(str(page_path), **_diff_vips_tiff_save_options())
        ...
        for page_path in page_paths:
            pil_pages.append(_open_diff_pil_page(page_path))
        pil_pages[0].save(
            str(temp_path),
            save_all=True,
...
def _create_diff_tiff(
    ...
    for page_index in range(page_count):
        ...
        if page_index < old_count:
            old_page = _load_diff_vips_page(old_file_path, page_index)
        if page_index < new_count:
            new_page = _load_diff_vips_page(new_file_path, page_index)
    ...
    for page_index, (old_page, new_page) in enumerate(zip(old_pages, new_pages)):
        # 寸法違いまたは片側にしかないページは完全一致にならないため、全画素判定を省略する
        detect_complete_match = (
            old_page is not None
            and new_page is not None
            and old_page.width == new_page.width
            and old_page.height == new_page.height
        )
        pages_are_identical = False
        if detect_complete_match:
            pages_are_identical = _diff_vips_pages_are_identical(old_page, new_page)
            # 一致判定で順次読み込みを完了したため、出力用に新画像だけを開き直す
            if pages_are_identical:
                old_page = None
                new_page = _load_diff_vips_page(new_file_path, page_index)
            else:
                # 差分画像の生成には両画像が必要なため、順次読み込みを先頭から作り直す
                old_page = _load_diff_vips_page(old_file_path, page_index)
                new_page = _load_diff_vips_page(new_file_path, page_index)
            old_pages[page_index] = old_page
        ...
        if pages_are_identical:
            # 完全一致時は追加・削除判定を省略し、新画像へメッセージだけを合成する
            diff_page = _add_diff_no_change_text(new_page)
        else:
            if old_page is None:
            ...
            diff_page = _make_diff_vips_page(old_page, new_page)
...
class ImageView(QGraphicsView):
    ...
    def __init__(self, parent=None):
        ...
        self._fit_mode = True
        self._fit_area_resize_in_progress = False
    ...
    def resize_window_to_displayed_image(self) -> bool:
        """Fit中の画像倍率を保ち、画像表示領域を現在の画像表示寸法へ合わせる。"""
        if not self._fit_mode:
            return False

        br = self._item.boundingRect()
        window = self.window()
        if br.isNull() or window is None:
            return False

        # scene座標の画像寸法へ現在の変換だけを適用し、実際の画面上の画像寸法を求める
        saved_transform = self.transform()
        displayed_rect = saved_transform.mapRect(br)
        viewport_size = self.viewport().size()
        # fitInViewが確保する最小余白は残し、縦横比による余分な余白だけを取り除く
        fit_padding = max(
            0.0,
            min(
                viewport_size.width() - displayed_rect.width(),
                viewport_size.height() - displayed_rect.height(),
            ),
        )
        target_view_size = QSize(
            max(1, int(math.ceil(displayed_rect.width() + fit_padding))),
            max(1, int(math.ceil(displayed_rect.height() + fit_padding))),
        )
        if not target_view_size.isValid():
            return False

        old_frame = window.frameGeometry()
        old_center = old_frame.center()

        # 最大化中も通常ウィンドウへ戻して、画像寸法に合わせたサイズを適用する
        self._fit_area_resize_in_progress = True
        try:
            ...
            viewport_size = self.viewport().size()
            chrome_width = max(0, window.width() - viewport_size.width())
            chrome_height = max(0, window.height() - viewport_size.height())
            new_width = target_view_size.width() + chrome_width
            new_height = target_view_size.height() + chrome_height
            window.resize(new_width, new_height)

            # リサイズアンカーの影響を除き、画像倍率と画像中心を押下前のまま維持する
            self.setTransform(saved_transform)
            self.centerOn(br.center())
        finally:
            self._fit_area_resize_in_progress = False

        # ウィンドウ中心を保ちつつ、利用可能な画面領域から外れない位置へ補正する
        screen = QGuiApplication.screenAt(old_center)
        if screen is None:
            screen = window.screen()
        if screen is None:
            screen = QApplication.primaryScreen()
        if screen is not None:
            ...
            target_x = old_center.x() - (new_frame.width() // 2)
            target_y = old_center.y() - (new_frame.height() // 2)
            max_x = max(available.left(), available.right() - new_frame.width() + 1)
            max_y = max(available.top(), available.bottom() - new_frame.height() + 1)
            window.move(
                min(max(target_x, available.left()), max_x),
                min(max(target_y, available.top()), max_y),
            )

        log_info(
            "ImageView fit_area_to_image target_view=%s actual_view=%s scale=%.5f window=%sx%s",
            _size_text(target_view_size),
            _size_text(self.viewport().size()),
    ...
    def resizeEvent(self, event):
        ...
        if self._fit_mode and self.has_image() and not self._fit_area_resize_in_progress:
            self.fit_in_view()
        self._schedule_preview_resize_update()
        if self.has_image():
    ...
    def _apply_sync_command(self, command: str, value: int = 0) -> bool:
        ...
        if command == "fit":
            if not self.view.has_image():
                return False
            if self.view.is_fit_mode() and self.view.resize_window_to_displayed_image():
                return True
