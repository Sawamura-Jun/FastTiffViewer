...
- ショートカット：**`c`** でアクティブなウィンドウを閉じます
- メニュー **`Spread(S)`** / ショートカット：**`s`** で同じモニター上の2つのウィンドウを左右に見開き配置します
...
- **Spread(S)** to arrange two windows side by side
...
- Shortcut: **`c`** closes the active window
- Menu **`Spread(S)`** / shortcut: **`s`** arranges two windows side by side on the same monitor
